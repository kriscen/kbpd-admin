...
import type { KbpdButtonGroupProps, ValueType } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@kbpd-core/icons';
import { cn, isFunction } from '@kbpd-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { KbpdRenderContent } from '../render-content';
import KbpdButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<KbpdButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
  ...
  >
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <KbpdRenderContent :content="btn.label" />
      </slot>
    </Button>
  </KbpdButtonGroup>
</template>
<style lang="scss" scoped>
.kbpd-check-button-group {
  display: flex;
  flex-wrap: wrap;
