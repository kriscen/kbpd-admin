...
import type { FormActions, FormSchema, KbpdFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@kbpd-core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
  ...
  StateHandler,
} from '@kbpd-core/shared/utils';

function getDefaultState(): KbpdFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<KbpdFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: KbpdFormProps | null = null;
  stateHandler: StateHandler;

  public store: Store<KbpdFormProps>;
  ...
  private prevState: KbpdFormProps | null = null;

  constructor(options: KbpdFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<KbpdFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: KbpdFormProps) => Partial<KbpdFormProps>)
      | Partial<KbpdFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<KbpdForm /> is not mounted');
    }
    return this.form;
  }
